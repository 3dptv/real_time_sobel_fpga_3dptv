...
void compose_raw_name_plus_nr_str (basename, str, nr, filename) // for Sobel RAW files
char	basename[256], str[256], filename[256];
int		nr;
{
  char	nr_ch[256];
  ...
  else 	sprintf (nr_ch, "%08d",  nr); // special for 2D PTV, Alex
...
//*************************************************************
void read_raw(int argc, char **argv)
{
	int	i_seq, i, j, n, trajid, maxvector, read;
	...
	// sprintf (filein,"img/%08d.raw", i_seq - 100000);
	compose_raw_name_plus_nr_str (seq_name[0], ".raw",i_seq, filein); //seq_name[i_img] for 3D, [0] for 2D
	// sprintf (fileout,"cam1.1%05d_targets", i_seq);
	// printf("%s -- > %s\n",  filein, fileout);
	FILEIN = fopen (filein, "rb");	if (! FILEIN) ;		//open the file in binary mode
	...
	do
	{
		...
		if (pImageInfo->dummy[0] != ntohl(0x00102030))
		{
			...
			*/
			...
			tmpx = (float)(*pTarget).sum_gu/(*pTarget).sum_g;
			tmpy = (float)(*pTarget).sum_gv/(*pTarget).sum_g;
			if((*pTarget).sum_g == 0 || tmpx >1280 || tmpy > 1024){
				break;
			}
			pix[0][i].x = tmpx;
			pix[0][i].y = tmpy;
			pix[0][i].n = (int)(*pTarget).sp8+(*pTarget).sp47+(*pTarget).sp03; // (int)(*pTarget).sp8<<8+(*pTarget).sp47<<4+(*pTarget).sp03;
			pix[0][i].nx = (int)(*pTarget).max_u-(*pTarget).min_u;
			pix[0][i].ny = (int)(*pTarget).max_v-(*pTarget).min_v;
			pix[0][i].sumg = (int)(*pTarget).sum_g;
			pix[0][i].pnr = i;
			pix[0][i].tnr = i;

			// fprintf(FILEOUT,"%4d %7.2f %7.2f %4d %4d %4d %5d %4d  \n", i, x, y,  n, nx, ny, sumg , i);
		}
		i++;
