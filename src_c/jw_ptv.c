...
{
	int     i, j, i_img, ok, k, nslices=19, slicepos=0;
	...
	for (i=seq_first; i<seq_last+1; i++)
	{
		...
		} else { /* display => 3 - means sequence for Sobel */
			/* Working version of Sequence from Sobel, reads _targets ASCII files
			for (i_img=0; i_img<n_img; i_img++) 
			{ 
				compose_name_plus_nr_str (seq_name[i_img], "_targets",i, filein);
				...
				for (j=0; j<num[i_img]; j++)
				{
					fscanf (FILEIN, "%4d %lf %lf %d %d %d %d %d\n",
						...
						&pix[i_img][j].sumg, &pix[i_img][j].tnr); // note that PNR is used twice. not sure if it should be tnr in latter
				}
				fclose (FILEIN);
			}
			*/
			/* Reads raw files directly, presently for a single camera only */
			read_raw(i); // fills in pix[0] from the i(th) file, 'i' is a sequence counter
